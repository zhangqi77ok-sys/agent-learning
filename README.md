...
│  │    • tool.fs (沙箱读写/列表/快照)    • tool.search (工作区 grep & find 检索)      │  │
│  │    • tool.git (真实版本控制)         • tool.terminal (无窗口静默执行)             │  │
│  │    • MCP Manager (标准 JSON-RPC 2.0 stdio 协议)                                   │  │
...
### 6. 真实双栈 TDD 自动化验证 (Dual-Stack Real TDD Engine)
* **拒绝假成功**：`RunTDDValidation` 支持自动探测 Go 原生测试 (`go test -v ./...`) 与 Node 前端测试 (`npm test`)；
* **无测试强报失败**：若工作区未检测到有效测试框架套件，直接返回失败并不准标记任务完成，从根源杜绝「无测试假绿灯」；测试失败实时同步至顶栏红灯徽章。

### 7. 项目宪法顶栏显式可见 (Visible Constitution)
