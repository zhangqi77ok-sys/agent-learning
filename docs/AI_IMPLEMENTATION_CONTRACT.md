...
- [ ] 未点「测速」的渠道卡片不出现「在线/极佳」。  
- [ ] 全文搜索 UI 无「OAuth」「快照时间旅行」暗示。  
- [ ] 无 Go 行为变化；`go test ./internal/config` 绿。
...
- [ ] 一次对话写 2 个文件，右侧自动打开其中一个 Diff，横幅显示 2 个路径。  
- [ ] 全部放弃后横幅清空、任务态不再 `pending_diff`。  
- [ ] 无新假按钮。
...
- [ ] 导入真实 SKILL.md 后新对话 system 含该 prompt（技能启用时）。  
- [ ] `@` 选中 `README.md` 后模型能读到文件内容，不是只有路径字。
...
- [ ] 保存带 env 的 MCP，json 文件里看得到 env。  
- [ ] 错误命令探活，UI 显示真实 stderr/错误字符串。
...
- [ ] `/tdd` 在本仓库：能看到 go test 输出摘要；明确 npm 跳过原因。  
- [ ] 失败时顶栏「测试未通过」。
...
- 思考块默认折叠，点击展开（现货常整段展开）。  
- 多工具默认折叠为「调用 N 个工具」，点击展开入参/输出。  
- 复制 / 重新生成保持接真会话（已有则补「折叠」即可）。
...
**完成定义**：设置打开时 Esc 关设置；设置里渠道子层先关子层。
...
- [ ] 是否改了 `archive/` 或把 HTML 原型当发货？若是，打回。  
- [ ] 是否新增假在线/假会话？若是，打回。  
- [ ] `SetWorkspace` 是否仍注册 search？  
- [ ] 用户能否不读源码完成「搜文件 → 打开 → 只读提问」？  
- [ ] 矩阵/README 有没有把未完成项标成完成？  
